...
def showCategorias():
    ...
    itensRecentes = session.query(Item).order_by(Item.id).limit(8)
...
def newUsuario(login_session):
    ...
    session.commit()
    usuario = session.query(User).filter_by(email=login_session['email']).one()
    return usuario.id
...
def showCategoria(categoria):
    ...
    pagina = "showCategoria.html"
    if "usuario_id" not in login_session:
        pagina = "showCategoriaPublica.html"
    return render_template(pagina, categorias=todasCategorias, categoria=umaCategoria, itens=seusItens)
...
def editCategoria(categoria):
    ...
    if login_session["usuario_id"] != umaCategoria.usuario_id:
        flash("Você não é o dono desta categoria.")
...
def deleteCategoria(categoria):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("A categoria foi excluída! Seus itens também foram excluídos!")
...
def showItem(categoria, item):
    ...
    if umItem is None:
        ...
        pagina = "showItem.html"
        if "usuario_id" not in login_session:
            pagina = "showItemPublica.html"
        return render_template(pagina, item=umItem)
